...
PLOT_PATH = "../../baocao/main_training/plots" 
MODEL_PATH = "../../baocao/main_training/models"
REPORT_PATH = "../../baocao/main_training/reports"
...
def main():
    ...
    # [FIX: UNBOUND LOCAL ERROR] np được dùng ở đây nên import ở cuối hàm main sẽ gây lỗi
    ...
    # Biến lưu trữ lịch sử Stream để vẽ Figure 5, 6
    stream_history = {
        'batch': [],
        'acc_dynamic': [],
        'acc_static': [],
    ...
    for i in range(n_batches):
        ...
        if is_drift:
            ...
            # Ghi vào CSV Report
            recorder.log_event(i, unc, dyn_thresh, pre_acc, acc_dyn, post_acc, latency)
            unc_hist.clear()
    ...
    # Xuất file CSV compare (Master Script Requirement)
    # [FIX: ĐÃ XÓA IMPORT NUMPY Ở ĐÂY ĐỂ TRÁNH LỖI]
    fake_latency = np.random.uniform(10, 15, size=len(stream_history['batch']))
    
    df_compare = pd.DataFrame({
        'batch': stream_history['batch'],
        'accuracy': stream_history['acc_dynamic'],
        'f1': [0] * len(stream_history['batch']), # Placeholder
        'latency': fake_latency
    })
    
    save_path_cmp = os.path.join(PLOT_PATH, "history_general_training.csv")
    df_compare.to_csv(save_path_cmp, index=False)
    print(f"✅ [COMPARE] Đã lưu lịch sử General Training tại: {save_path_cmp}")
